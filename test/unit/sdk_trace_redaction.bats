...
trace_file="${TEST_TMPDIR}/sdk.trace.log"
secret_marker="SHOULD_NOT_APPEAR_IN_TRACE"

printf ' -> sdk does not leak payload expansions in xtrace\n'
(
	set -euo pipefail
...
if [ ! -s "${trace_file}" ]; then
	test_fail "expected trace file to be non-empty"
fi

# Critical assertion: secret marker must not appear in trace.
if grep -q -- "${secret_marker}" "${trace_file}"; then
	test_fail "secret marker leaked into xtrace output"
fi

# Sanity: confirm we captured function names (trace still useful).
if ! grep -q -- "mcp_args_get" "${trace_file}" && ! grep -q -- "mcp_meta_get" "${trace_file}"; then
	test_fail "expected trace to include some sdk function call lines"
fi

printf 'sdk trace redaction tests passed.\n'
